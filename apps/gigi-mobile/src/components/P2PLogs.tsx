  ...
  return (
    <div className="flex flex-col h-full p-4">
        ...
        <div className="flex gap-2 flex-wrap">
          <button 
            onClick={() => {
              ...
              addLog('test-event', { message: 'This is a test event to verify logging works' }, 'info');
            }}
      ...
      <div className="flex-1 overflow-y-auto bg-gray-50 rounded p-2 min-h-0">
        {logs.length === 0 ? (
          <p className="text-gray-500 text-center py-8">No logs yet. Events will appear here.</p>
        ) : (
          <div className="space-y-2">
            {logs.map((log) => (
              <div key={log.id} className="bg-white p-3 rounded shadow-sm border-l-4 border-blue-400">
                <div className="flex justify-between items-start mb-2">
                  <span className={`font-mono text-sm font-semibold ${getLogColor(log.type)}`}>
                    {log.event}
                  </span>
