...
export class AgentMessagingClient {
  ...
  private async joinAgentGroup(): Promise<void> {
    try {
      ...
      console.log(
        `Joined agent group: ${AgentMessagingClient.AGENT_GROUP_NAME}`
      )
    } catch (error) {
      console.warn(
        `Failed to join agent group: ${error instanceof Error ? error.message : 'Unknown error'}`
      )
    }
  }
  ...
  private handleAgentGroupMessage(event: any): void {
    if (
      ...
      event.content.text.startsWith('{"type":"')
    ) {
      try {
        const ampMessage = JSON.parse(event.content.text) as AmpMessage
        if (ampMessage.type) {
          console.log(
            `[AMP GROUP] ${event.from_nickname} sent an AMP message: ${ampMessage.type}`
          )
        ...
        console.log(
          `[AGENT GROUP] ${event.from_nickname}: ${event.content.text}`
        )
      }
      ...
      console.log(
        `[AGENT GROUP] ${event.from_nickname} shared a file: ${event.content.filename} (${event.content.fileSize} bytes)`
      )
    }
  }
  ...
  private handleDirectMessage(event: any): void {
    try {
      ...
      if (ampMessage.type) {
        console.log(
          `[AMP DIRECT] ${event.from_nickname} sent an AMP message: ${ampMessage.type}`
        )
        this.messageRouter.routeMessage(ampMessage)
      }
    } catch {
  ...
  async sendTextMessage(
    ...
    const target =
      targetType === 'all'
        ? { type: 'all' as const }
        : { type: 'specific' as const, agentIds }
    const ampMessage = AmpMessageFactory.createTextMessage(message, target, {
      id: this.peerId,
      name: this.nickname,
      type: 'owner' as const,
    })
  ...
  private async sendAmpMessage(ampMessage: AmpMessage): Promise<void> {
    try {
      ...
      console.error(
        `Error sending AMP message: ${error instanceof Error ? error.message : 'Unknown error'}`
      )
      throw error
    }
  }
