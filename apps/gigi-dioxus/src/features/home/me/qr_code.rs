...
pub fn QRCodeTab(props: QRCodeTabProps) -> Element {
    let qr_data = format!("{{\"name\":\"{}\",\"peerId\":\"{}\"}}", props.name, props.peer_id);

    let svg_content = use_memo(move || {
        let code = QrCode::new(qr_data.as_bytes());
        match code {
            Ok(qr) => {
                let rendered = qr.render::<svg::Color>()
    ...
    rsx! {
        div { class: "p-6 bg-gray-50 h-full",
            div { class: "text-center space-y-6",
                ...
                div { class: "space-y-3",
                    button { class: "w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl transition-colors duration-200 flex items-center justify-center gap-2",
                        onclick: move |_| {
