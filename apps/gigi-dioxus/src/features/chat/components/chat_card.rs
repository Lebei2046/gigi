...
use crate::features::chat::chat_state::{Peer, Group, Conversation};
...
pub fn PeerChatCard(
    ...
    let last_message_time = conversation.as_ref().and_then(|c| c.last_message_time.clone());
    let first_letter = peer.nickname.chars().next().unwrap_or('?').to_uppercase().to_string();
                ...
                div { class: "flex justify-between items-start",
                    div { 
                        ...
                        div { class: "flex-1 min-w-0",
                            ...
                            div { class: "text-xs text-gray-500 font-mono truncate", "{peer.id}" }
                            if let Some(message) = last_message {
                                div { class: "text-sm text-gray-600 truncate mt-1", "{message}" }
                            }
...
pub fn GroupChatCard(
    ...
    rsx! {
        div { 
            ...
            div { 
                class: "flex justify-between items-start p-4 cursor-pointer",
                div { class: "flex-1",
                    div { class: "flex items-center gap-2 mb-1",
                    ...
                    if let Some(message) = last_message {
                        div { class: "text-sm text-gray-600 mt-2 truncate ml-12",
                            "{message}"
                        }
                    }
                }
