...
impl MessageService {
    ...
    pub async fn load_message_history(&self, peer_nickname: &str, limit: u32, offset: u32) -> Vec<Message> {
        println!("Loading message history for chat: {} with limit: {}, offset: {}", peer_nickname, limit, offset);
        
        match PersistenceService::load_messages(peer_nickname, limit as usize, offset as usize).await {
            Ok(stored_messages) => {
    ...
    pub async fn save_message_full(&self, message: &Message, chat_id: &str, from_nickname: &str, to_nickname: &str, is_own: bool) -> Result<(), String> {
    ...
    pub async fn save_message(&self, message: &Message, chat_id: &str) -> Result<(), String> {
        self.save_message_full(message, chat_id, "You", "", true).await
    }
