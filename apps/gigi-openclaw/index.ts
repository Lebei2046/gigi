  ...
  register: (api) => {
    ...
    api.registerTool({
      ...
      execute: async (toolCallId, params) => {
        ...
        return {
          content: [
            { type: 'text', text: `Joined group: ${params.groupName}` },
          ],
          details: {
            success: true,
            message: `Joined group: ${params.groupName}`,
          },
        };
      },
    });
    ...
    api.registerTool({
      ...
      execute: async (toolCallId, params) => {
        ...
        return {
          ...
          details: {
            success: true,
            message: `Left group: ${params.groupName}`,
          },
        };
      },
    });
      ...
      execute: async (toolCallId, params) => {
        ...
        return {
          content: [
            {
              type: 'text',
              text: `Found ${groups.length} groups: ${groups.map((g) => g.name).join(', ')}`,
            },
          ],
          details: { groups },
        };
        ...
        try {
          ...
          return {
            content: [
              {
                type: 'text',
                text: `Error generating mnemonic: ${error instanceof Error ? error.message : 'Unknown error'}`,
              },
            ],
            details: {
