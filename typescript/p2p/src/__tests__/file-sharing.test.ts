...
describe('FileSharingManager', () => {
  ...
  it('should share a file and return a shared file object', async () => {
    ...
    const sharedFile = await fileSharingManager.shareWithContent(
      'test.txt',
      content,
      'text/plain'
    );
  ...
  it('should throw an error when sharing a non-existent file', async () => {
    ...
    await expect(
      fileSharingManager.share('./non-existent-file.txt')
  ...
  it('should return undefined for non-existent share code', () => {
    const foundFile = fileSharingManager.getByShareCode(
  ...
  it('should revoke a shared file', async () => {
    ...
    expect(
      fileSharingManager.getByShareCode(sharedFile.shareCode)?.info.revoked
    ).toBe(false);
  ...
  it('should list all shared files', async () => {
    ...
    await fileSharingManager.shareWithContent(
      'test1.txt',
      content1,
      'text/plain'
    );
  ...
  it('should save a file', async () => {
    const filePath = await fileSharingManager.saveFile('test.txt', [
